...
import cv2
from ldm.annotator.util import nms
...
class DDIMSampler(object):
    ...
    def make_schedule(self, ddim_num_steps, ddim_discretize="uniform", ddim_eta=0., verbose=True, control_start_step=0):
    ...
    def sample(self,
        ...
        self.make_schedule(ddim_num_steps=S, ddim_eta=eta, verbose=verbose, control_start_step=kwargs['control_start_step'])
        # sampling
        C, H, W = shape
    ...
    def ddim_sampling(self, cond, shape,
        ...
        for i, step in enumerate(iterator):
            ...
            curr_cond = cond.copy()
            curr_un_cond = unconditional_conditioning.copy()
            
            
            # if (i + 1 < kwargs['control_start_step']) or (i + 1 > kwargs['control_end_step']):
            #     curr_cond["c_concat"] = None
            #     curr_un_cond["c_concat"] = None
                
                
            img, pred_x0, layers_cross_attn_maps = self.p_sample_ddim(img, curr_cond, ts, index=index, use_original_steps=ddim_use_original_steps,
                                      quantize_denoised=quantize_denoised, temperature=temperature,
                                      noise_dropout=noise_dropout, score_corrector=score_corrector,
                                      corrector_kwargs=corrector_kwargs,
                                      unconditional_guidance_scale=unconditional_guidance_scale,
                                      unconditional_conditioning=curr_un_cond,
                                      dynamic_threshold=dynamic_threshold)
            if callback: callback(i)
            if img_callback: img_callback(pred_x0, i)
    ...
    def get_pseudo_mask(self, all_cross_attn_maps, noun_idx, shape):
        ...
        # contour_masks = binary_masks
        # foreground_masks = binary_masks
        ...
        # binary_masks = np.concatenate([get_max_connected(bm)[None, ...] for bm in binary_masks], axis=0)
        # binary_masks = torch.from_numpy(binary_masks / 255).unsqueeze(1).repeat(1, 3, 1, 1)
        # return binary_masks.float()



model_name = 'control_v11p_sd15_scribble'
model = create_model(f'configs/ldm_ad/cldm_v15.yaml').cpu()
model.load_state_dict(load_state_dict('./checkpoints/v1-5-pruned.ckpt', location='cuda'), strict=False)
model.load_state_dict(load_state_dict(f'./checkpoints/{model_name}.pth', location='cuda'), strict=False)
model = model.cuda()
ddim_sampler = DDIMSampler(model)
# ddim_sampler = DDIMSampler('configs/ldm_ad/cldm_v15.yaml', './checkpoints/v1-5-pruned.ckpt', 
...
cond = {"c_concat": None, "c_crossattn": [ddim_sampler.model.get_learned_conditioning([prompt + ', ' + a_prompt] * num_samples)]}
un_cond = {"c_concat": None, "c_crossattn": [ddim_sampler.model.get_learned_conditioning([n_prompt] * num_samples)]}

H, W = image_resolution, image_resolution
shape = (4, H // 8, W // 8)
ddim_sampler.model.control_scales = [strength * (0.825 ** float(12 - i)) for i in range(13)] if guess_mode else ([strength] * 13)
img, intermediates = ddim_sampler.sample_create_image_mask(ddim_steps, num_samples,
...
imgs = ddim_sampler.model.decode_first_stage(img)
...
for i, (img, mask) in enumerate(zip(imgs, masks)):
    ...
    # extracted_img = cv2.resize(img[y:y+h, x:x+w], (W, H))
    # extracted_mask = cv2.resize(mask[y:y+h, x:x+w], (W, H))
    # imgs[i] = extracted_img
    # masks[i] = extracted_mask
from PIL import Image
for i, sample in enumerate(imgs):
    Image.fromarray(sample).save(f'samples/{seed}_{i}.jpg')