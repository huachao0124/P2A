...
class DDIMSampler(BaseModule):
    def __init__(self, model, ldm_pretrain, control_pretrain, schedule="linear", out_indices=(0, 1, 2, 3), **kwargs):
        super().__init__()
    ...
    def sample_create_image_mask(self,
        ...
        contour_masks, foreground_masks, contours = self.first_step(conditioning, size,
                                                    callback=callback,
                                                    img_callback=img_callback,
                                                    quantize_denoised=quantize_x0,
                                                    mask=mask, x0=x0,
    ...
    def first_step(self, cond, shape,
        ...
        contour_masks, foreground_masks, contours = self.get_pseudo_mask(intermediates['cross_attn_maps'], noun_idx=[6] * img.shape[0], shape=img.shape)
        
        return contour_masks, foreground_masks, contours
    ...
    def get_pseudo_mask(self, all_cross_attn_maps, noun_idx, shape):
        ...
        pseudo_masks = (pseudo_masks - pseudo_masks.min()) / (pseudo_masks.max() - pseudo_masks.min())
        ...
        def get_max_connected(m):
            ...
            return contour_mask, foreground_mask, contours[max_idx]
        ...
        for bm in binary_masks:
            ...
            # bm[bm < thresh] = 0
            # bm[bm >= thresh] = 255
            cm, fm, contour = get_max_connected(bm)
        ...
        return contour_masks.float(), foreground_masks.float(), contours
